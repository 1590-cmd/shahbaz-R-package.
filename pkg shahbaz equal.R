...
# size(P) Rule-II These designs Generated with R package.
...
design<-function(H){
  ...
  setMethod("show", "CRMD_design", function(object) {
    ...
    cat(row, "\n")
    cat("This Design are generated through above computed sets of shifts for", "v=" ,object$R[1], "and","k=",object$R[2], "\n")
    row <- paste(rep("=", 51), collapse = "")
    cat(row, "\n")
...
Create_Designs<-function(v,p,C,i){
   if((v)%%(p)==0 & (i=(v)/(p)) & (i%%1==0) &(i>0) ){
    ...
    cat("MCBRMDs is possible for","v =",v,"P =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    (H<-RMD_equalsize(v=v,p=p,i=i,C=1))
    
}else
  if((v-1)%%(p)==0 & (i=(v-1)/(p))  & (i%%1==0) &(i>0)) {
   
    cat("MCPBRMDs-I is possible for","v =",v,"p =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    
    (H<-RMD_equalsize(v=v,p=p,i=i,C=2))        
  }else
    if((v-2)%%(p)==0 & (i=(v-2)/(p))  & (i%%1==0) &(i>0)){
        
    cat("MCPBRMDs-II is possible for","v =",v,"p =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    ...
    cat("MCWBRMDs-I is possible for","v =",v,"p =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    (H<-RMD_equalsize(v=v,p=p,i=i,C=4))        
  }else
    if((v+2)%%(p)==0 & (i=(v+2)/(p))  & (i%%1==0) &(i>0)){
            
    cat("MCWBRMDs-II is possible for","v =",v,"P =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    (H<-RMD_equalsize(v=v,p=p,i=i,C=5))
            
  }else
    if((v+1)%%(p)==0 & (i=(v+1)/(p))  & (i%%1==0) &(i>0)) {
              
    cat("MCNSBRMDs is possible for","v =",v,"p =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    (H<-RMD_equalsize(v=v,p=p,i=i,C=6))        
  }else
    if((v)%%(p)==0 & (i=(v)/(p)) & (i%%1==0) &(i>0) ){
                
    cat("MCSPBRMDs-I is possible for","v =",v,"p =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    (H<-RMD_equalsize(v=v,p=p,i=i,C=7))        
                
  }else
    if((v-1)%%(p)==0 & (i=(v-1)/(p)) & (i%%1==0) &(i>0)) {
                  
    cat("MCSPBRMDs-II is possible for","v =",v,"p =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    (H<-RMD_equalsize(v=v,p=p,i=i,C=8))        
  }else
    if((v+2)%%(p)==0 & (i=(v+2)/(p)) & (i%%1==0) &(i>0) ){
                    
    cat("MCSBGRMDs-I is possible for","v =",v,"P =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
    (H<-RMD_equalsize(v=v,p=p,i=i,C=9))
                    
  }else
    if((v+3)%%(p)==0 & (i=(v+3)/(p)) & (i%%1==0) &(i>0) ) {
                      
    cat("MCSBGRMDs-II is possible for","V =",v,"P =",p,"i =",i,"and","n =",((v-1)*i),rep("", 40))
...
are not possible for ","v =",v,"and", "P =",p,"i =",i,rep("", 40))
